...
system("mkdir /var/www/html/midterm/$class");
system("mkdir /var/www/html/midterm/$class\/$examine");
...
while (<IN>) {
    if (/$login/) {
        chomp();
        @fields = split(/:/);
        $passwdInFile = $fields[1];
	print "login=$login; passwdInFile=$passwdInFile; passwd=$passwd<br >\n";
        #$fullname = $fields[5];
	$found = 1;
        last;
    }
...
$file= "/var/www/html/midterm/$class\/$examine\/$filename";
#  relative path does not work Try full path
#$file= "/var/www/html/midterm/$class\/$examine\/$filename";
if (open my $fh, '>', $file) {
    # open was successful
    print $fh $buffer;
...
close(OUT);
$command="/bin/mail -s \'$examine from " . $login . "\' -c $login\@uccs.edu cchow\@uccs.edu < /var/www/html/midterm/$class\/$examine\/$filename";
system("$command");
print "$command\n";
print "</BODY></HTML>"; 
